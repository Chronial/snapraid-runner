...
def snapraid_command(command, args={}, ignore_errors=False, stdout_log_level=None):
    """
    ...
    out = []
    threads = [
        tee_log(p.stdout, out, stdout_log_level or logging.OUTPUT),
...
def run():
    ...
    if config["snapraid"]["smart"]:
        ...
        try:
            snapraid_command("smart", stdout_log_level=logging.SMART)
        except subprocess.CalledProcessError as e:
            logging.error(e)
            finish(False)
        logging.info("*" * 60)
